...
tresult PLUGIN_API CrossDelayProcessor::setupProcessing(ProcessSetup& setup)
{
    ...
    delayBufferL.resize(delayLineSizeInSamples);
    delayBufferR.resize(delayLineSizeInSamples);
...
tresult PLUGIN_API CrossDelayProcessor::process(ProcessData& data)
{
    ...
    {
        for (int32 channel = 0; channel < numChannels; ++channel)
            memset(outputs[channel], 0, numSamples * sizeof(Sample32));
    }
    else
    {
...
void PLUGIN_API CrossDelayProcessor::processBlock(Sample32** inputs, Sample32** outputs, int32 numSamples)
{
    ...
    for (int32 sample = 0; sample < numSamples; ++sample)
    {
        ...
        outputL[sample] = (1.0 - mixL) * inputL[sample] + mixL * delayedL;
        outputR[sample] = (1.0 - mixR) * inputR[sample] + mixR * delayedR;
